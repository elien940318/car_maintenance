...
└── packages/
    └── (보류)          # 공유 타입 패키지 미사용 — #14 결정 (2026-06-15)
```

> 모노레포 도구: pnpm workspaces (또는 단순 멀티 디렉터리 구성으로 시작)
>
> **API 타입 공유 방침 (#14, 2026-06-15)**: 응답 read-model DTO를 `packages/types` 공유 패키지로 두지 않고 **web·api 각 앱이 독립적으로 타입 정의**한다. 초기 구성을 단순화하되, 스키마 변경 시 양쪽 수동 동기화가 필요하므로 응답 형태 변경은 회귀 표면으로 관리한다. (향후 드리프트가 커지면 공유 패키지로 추출 가능)
...
| OD-5 (부품 프리셋) | **Prisma seed 제공** | `prisma/seed.ts`에 전체 연료×변속기 조합 프리셋 ~90개 시드(투싼 NX4 HEV 17개 포함). (#10, 2026-06-15) |
